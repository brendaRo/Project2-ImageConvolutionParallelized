...
int main(int argc, char *argv[]){
	...
	for(it= 1; it <= atoi(iterations); it++){ //chunk
	    int iter;
            iter = 10^(it);
            printf("Iterations %d:\n", iter);
    ...
    //convolution of the gray image and the filter
            ...
            #pragma omp parallel for shared(new_img_gray, gray_img_size, gray_img, filter, w, filtsize) schedule(dynamic, iter)
    ...
    //time
            printf("Total_convolution_time Th%d:_%f_seconds\n", g, (double)(t1-t0));
        } //exe
    } //threads
	} //chunk
